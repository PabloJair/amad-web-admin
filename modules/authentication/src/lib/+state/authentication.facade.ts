...
import { Login2fRequest, LoginRequest } from '@amad-web-admin/modules/network';
...
export class AuthenticationFacade {
  ...
  loaded$ = this.store.pipe(
    select(authenticationFeature.selectLoaded),
    skip(1),
  );
  successLogin$ = this.store.pipe(
    select(authenticationFeature.selectUserInformation),
    skip(1),
  );
