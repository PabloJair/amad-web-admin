...
import { StatusRol, UserEdit, UserRolItem, UserStatus } from '@amad-web-admin/shared';
...
export class UserEditComponent implements AfterViewInit, OnDestroy {
  ...
  private setupUser() {
    ...
    this.editUserForm.controls.estatus.setValue(this.userItem.status == UserStatus.ENABLE);
  ...
  ngAfterViewInit(): void {
    ...
    this.successEditUser$$ = this.userFacade.successAddUser$.subscribe((value) => {
      if (value) {
        this.dialogService.showSuccess('Atención', 'Usuario actualizado correctamente ');
  ...
  edit() {
    const user: UserEdit = {
      ...
      status: this.editUserForm.controls.estatus.value ? UserStatus.ENABLE : UserStatus.DISABLE,
